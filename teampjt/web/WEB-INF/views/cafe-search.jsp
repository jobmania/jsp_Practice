...
<div class="container" style="width:70%">
    ...
    <div class="d-flex justify-content-end">
        <a class="btn btn-primary" href="/cafe?page=1">카페 리스트로 돌아가기</a>
    </div>
    ...
    <table class="table table-striped" , id="dinerTable">
        ...
        <tbody>
        <c:if test="${empty cafes}">
            <tr>
                <td colspan="5">No data available</td>
            </tr>
        </c:if>

        <c:forEach items="${cafes}" var="cafe" varStatus="status">
            ...
            <tr>
                ...
                <td><a class="btn btn-primary" href="/map?address=${cafe.address}&id=${cafe.id}&board=cafe">상세 보기</a></td>
            </tr>
        </c:forEach>
        </tbody>
    </table>
    ...
    <!-- 페이징 처리 -->
        <nav aria-label="Page navigation example">
            <ul class="pagination">
                ...
                %>
                <li class="page-item"><a class="page-link" href="/cafe/search?page=<%= currentPage-1 %>&search_keyword=<%=search_keyword%>&search_target=<%=search_target%>">previous</a></li>
                <% } %>
                ...
                <% for (int i = startPage; i <= endPage; i++) {
                    ...
                    if (i == currentPage) { %>
                <li class="page-item active"><a class="page-link" href="/cafe/search?page=<%= i %>&search_keyword=<%=search_keyword%>&search_target=<%=search_target%>"><%= i %></a></li>
                <% } else { %>
                <li class="page-item"><a class="page-link" href="/cafe/search?page=<%= i %>&search_keyword=<%=search_keyword%>&search_target=<%=search_target%>"><%= i %></a></li>
                <% } %>
                <% } %>


                <% if (currentPage != totalPages && totalPages!=0 ) { %>
                <li class="page-item"><a class="page-link" href="/cafe/search?page=<%= currentPage+1 %>&search_keyword=<%=search_keyword%>&search_target=<%=search_target%>">next</a></li>
                <% } %>
            </ul>
        </nav>
